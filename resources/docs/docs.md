...
The `home-routes` handler in the `weather-4.routes.home` namespace
...
The `home-page` function will in turn call the `weather-4.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `weather-4.handler` namespace:
...
Request middleware functions are located under the `weather-4.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `weather-4.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `weather-4.core` in some other namespace.
